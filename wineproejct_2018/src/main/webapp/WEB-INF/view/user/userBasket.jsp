			...
			<div class="wrapper">
				...
				<div class="myP-header">
					...
					<span id="myP-grade">${user.rating }</span> <span class="myP-guide">
						고객님의 등급:<br> 다음 등급업까지 남은 구매금액은 <c:if
							test="${user.totalAmount < 200000 }">
							${200000 - user.totalAmount }
						</c:if> <c:if test="${user.totalAmount > 200000 }">
							${400000 - user.totalAmount }
						</c:if> <c:if test="${user.totalAmount > 400000 }">
							0
						</c:if> 원입니다.
					</span>
				</div>
